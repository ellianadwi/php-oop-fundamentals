...
class Validator
{
    ...
    /**
     * Validation data against a set of rules and set errors in the $this->errors
     *array
     *@param array $data
     *@param array $rules
     *@return boolean
    ...
    /**
     * Validate the $value ofitem to see if it is present and not empty
     * @param string $item
     * @param string $value
    ...
    private function validateMin ($item, $value, $parameter)
    {
        if (strlen($value) >= $parameter == false) {
            $this->errors[$item][] = 'The ' . 'field should have a minimum length of ' . $parameter;
            return false;
        }
